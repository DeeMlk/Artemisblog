...
```bash
# Sintaxe de todos os arquivos PHP (verificado: passa limpo nos 40 arquivos)
...
- **Docblocks pré-canvas corrigidos** (`inc/router.php`, `inc/setup.php`,
  ...
  header/rodapé do tema anfitrião; o `router.php` chegava a afirmar que os templates
  chamavam `get_header()`/`get_footer()`, o que nunca foi verdade no modo canvas.
...
Branch de desenvolvimento designada: **`claude/artemis-blog-plugin-docs-hdr53o`**.
...
Antes de escrever código numa mudança de mais de um arquivo, mostre o plano primeiro.
